...
\title{CLEVER: Workload Diversity Nullifies Semantic Eviction
in LLM Caches}
...
approximate nearest neighbor, HNSW, LMSYS, LLM-as-judge,
quality-adjusted hit rate, negative results}
...
\begin{document}
\begin{abstract}
The latency and computational cost of Large Language Model (LLM) serving can be significantly reduced using semantic caching, which retrieves historical responses for semantically similar queries. While optimizing cache indexing and query routing is well-studied, cache eviction (the problem of which embeddings to discard when capacity is reached) remains underexplored. We introduce CLEVER, a framework that evaluates Approximate Nearest Neighbor (ANN) indexes, implements a cost-aware adaptive router, and proposes a novel semantic-aware eviction policy that scores entries by the ratio of their semantic redundancy to their utility. We evaluate six eviction policies (LRU, LFU, ARC, GDSF, the recent SISO, and our semantic policy) on three workloads (LMSYS-Chat-1M, Quora Question Pairs, MOSS), under two embedding models, and audit the semantic validity of cache hits with an LLM judge.

We demonstrate that HNSW is the Pareto-optimal index and that adaptive routing achieves 60.33\% mean latency savings across five seeds. Our central findings, however, are negative. First, no policy beats LFU at any cache size on any workload: ARC converges to LFU's decisions, SISO---designed specifically for LLM serving---is the worst policy on both sparse workloads, and every policy gap shrinks monotonically with capacity, vanishing by a 30\% cache. The ordering replicates under a second encoder, although hit thresholds themselves do not transfer across encoders and must be recalibrated per encoder. Second, an LLM judge finds that most cache hits are not semantically equivalent queries: quality-adjusted hit rates collapse from 51--60\% to 1--2\% on LMSYS and QQP and from 98\% to 25\% on MOSS, and the residual differences between policies fall within binomial noise. Meanwhile the semantic policy adds 5.83--8.24$\times$ per-query overhead relative to LRU. We conclude that the binding constraint in semantic caching is hit \emph{quality}, not eviction: LFU is the dominant, cost-effective eviction strategy, and effort is better spent on per-encoder threshold calibration than on semantically informed eviction.
\end{abstract}
\maketitle
...
While systems like GPTCache \cite{bang2023gptcache} default to standard heuristics like Least Recently Used (LRU) or Least Frequently Used (LFU), these policies ignore the spatial relationships of the embeddings. Intuitively, a semantic cache should optimize for ``semantic coverage.'' If two cached entries are highly similar, they are redundant; evicting one minimally impacts the cache's ability to serve future similar queries. Conversely, an isolated entry provides unique coverage and should be protected.

We hypothesize that an eviction policy explicitly aware of this semantic redundancy will outperform naive LRU/LFU by maintaining a more diverse, representative cache state. To test this, we developed CLEVER (Cluster-Level Eviction for Vector Embedding Retrieval). CLEVER benchmarks standard ANN structures, proposes an adaptive cost-based router, and introduces an $O(S)$ incremental symmetric redundancy graph to score cache entries by their redundancy-to-utility ratio.

However, our empirical evaluation refutes our initial hypothesis for general-purpose conversational workloads. On the 579,753-query LMSYS-Chat-1M dataset, the extreme diversity of real-world human-LLM interactions results in a highly sparse redundancy graph, and the semantic eviction policy provides zero hit-rate benefit over LFU while incurring significant computational overhead. We then stress-test this negative result along three axes. We add three baselines from the caching literature (ARC, GDSF, and the recent LLM-serving policy SISO) and two additional workloads (Quora Question Pairs and MOSS), sweep cache capacity, and replicate the entire matrix under a second embedding model: LFU is never beaten, and every policy gap shrinks as capacity grows. Finally, we audit what the hit rate actually measures. An LLM judge, validated against an independently hosted judge, finds that most cache hits at the standard similarity threshold are not semantically equivalent queries: quality-adjusting the hit rate collapses it by an order of magnitude and erases the remaining differences between policies.

\textbf{Contributions.} This paper makes the following contributions:
\begin{enumerate}
\item \textbf{ANN Index Benchmark:} A systematic comparison of FAISS index structures (Flat, HNSW, IVF, LSH) under realistic semantic caching workloads, establishing HNSW as the Pareto-optimal choice (Recall@1 $= 0.998$, $P50 = 0.171$ ms at $M{=}64$, $ef{=}256$).
\item \textbf{Adaptive Cost-Based Router:} A threshold-selection algorithm that maximizes a joint objective of latency savings and semantic quality, achieving 60.33\% mean latency savings across five seeds on LMSYS-Chat-1M.
\item \textbf{Semantic Eviction Policy:} A novel scoring function and $O(S)$ incremental symmetric redundancy graph that grades cache entries by redundancy-to-utility ratio with $\mu$-smoothing to guarantee graceful fallback.
\item \textbf{Negative Empirical Result:} A rigorous demonstration that, on high-diversity conversational workloads, semantic eviction provides no hit-rate benefit over LFU while adding $5.83$--$8.24\times$ per-query overhead, establishing clear workload-dependency conditions for when semantic eviction is cost-effective.
\item \textbf{Six-Policy, Cross-Encoder Robustness Study:} An extension of the negative result to ARC, GDSF, and SISO across three workloads, three cache capacities, and two embedding models, showing that LFU is never beaten, that all policy gaps vanish as capacity grows, and that semantic-cache hit thresholds do not transfer across encoders and must be recalibrated per encoder.
\item \textbf{LLM-Judged Quality Adjustment:} An audit of hit quality showing that most cache hits at the standard threshold are not semantically equivalent queries. Quality-adjusted hit rates collapse from 51--60\% to 1--2\% on LMSYS and QQP, policy differences flatten into binomial noise, and near-zero embedding distance does not imply equivalence on templated prompts.
\end{enumerate}

\section{Related Work}
\subsection{Semantic Caching for LLMs}
Semantic caching has emerged as a practical cost-reduction technique for LLM serving. GPTCache \cite{bang2023gptcache} provides a modular open-source architecture for embedding queries and searching via vector databases such as FAISS \cite{johnson2019faiss} or Milvus, relying on static similarity thresholds and classic eviction algorithms (LRU/LFU). More recently, Liu et al.\ \cite{liu2026semantic} formalize semantic cache eviction as a Combinatorial Multi-Armed Bandit (CMAB) problem, balancing mismatch costs and serving costs online without prior knowledge of query distributions. While their approach focuses on online regret minimization and combinatorial optimization, our work investigates the deterministic, offline structural properties of the cache graph itself, specifically whether maintaining an explicit spatial redundancy graph justifies its overhead on highly diverse, real-world conversational workloads.
...
CLEVER is built upon three modular components evaluated in sequence.
...
\subsection{Semantic Eviction Policy}
Our novel contribution is an eviction policy that scores entries based on the ratio of their semantic redundancy to their historical utility.
...
$\theta_C = 0.86$ cosine ($L_2^2 < 0.28$) absorbs one unit of cluster mass; an insert that opens a new semantic region receives temporary infinite-access-count protection. Every 1{,}000 requests a maintenance round decays all cluster sizes by $1.1\times$ and resets access counts; eviction removes the entry with the lexicographically smallest (cluster size, access count) pair. SISO's original cache manager re-clusters query logs offline; our implementation maps its merge step to online per-insert merging with the search bounded at 1{,}024 sampled entries, and drives its M/D/1-based adaptive retrieval threshold $\theta_R$ from a sliding-window hit rate. As in the original design, $\theta_R$ does not influence victim selection.

All six policies (LRU, LFU, Semantic, ARC, GDSF, SISO) run under the identical harness (\texttt{scripts/08\_run\_eviction.py}): the policy only observes insert/access/evict events and selects victims, while admission, lookup, the
$L_2^2 = 0.90$ hit threshold, and the 30\% warmup prefix are fixed by the cache.
...
dimensions) \cite{li2023gte}, is strongly anisotropic: random query pairs
...
\emph{below} the random-pair mean under gte-base, and essentially every pair
...
cached entries are neighbors iff one would be a hit for the other). SISO's
...
Each pair is scored by an independent LLM judge under a fixed binary
rubric. The system prompt casts the judge as a ``strict evaluator of
...
\noindent %
\begin{table}[h]
\caption{Hyperparameter configuration for CLEVER's semantic
eviction policy and adaptive router.}
...
\item \textbf{Normalization:} All vectors are $L_2$-normalized so that $L_2^2$ distance equals one minus cosine similarity, as required by the adaptive router.
...
\item \textbf{Hardware:} Benchmarks and routing evaluations are conducted on standard CPU architectures (32GB RAM), while the computationally intensive eviction multi-seed evaluations utilize Great Lakes HPC (Largemem partition, 16 CPUs, 64GB RAM).
...
\subsection{Index Benchmarking}
Index benchmarking was conducted on 499K vectors drawn from the full embedding corpus. At this scale, HNSW ($M=64$, $ef=256$) achieves a Recall@1 of 0.998 with a P50 search latency of 0.171 ms, versus 0.182 ms for exact Flat search. IVF suffers from severe recall degradation at low probe counts (Recall@1 = 0.641 at nprobe=1), and LSH fails to exceed 0.75 recall, rendering it non-viable for semantic caching.

\begin{figure}[h!]
  \centering
  \includegraphics[width=\columnwidth]{hnsw_pareto_tradeoff}
  \caption{Recall@1 vs.\ P50 latency across FAISS index types.
  HNSW is the Pareto-optimal choice; IVF and LSH fall below
  viable recall thresholds.}
  \label{fig:hnsw}
\end{figure}

\subsection{Adaptive Routing}
The adaptive router identifies a practical operating region around $\theta \in [0.76, 0.79]$. In five-seed evaluation under random cache fill, the mean selected threshold is $\theta = 0.772 \pm 0.015$, yielding a 60.43\% hit rate, average cosine quality of 0.806, and 60.33\% latency savings. For reference, the fixed-threshold single-seed sweep at $\theta = 0.70$ yields 71.56\% hit rate, cosine quality 0.858, and 71.46\% latency savings. Pushing $\theta > 0.90$ increases hit rate but drives quality toward the acceptance floor.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{router_threshold_sweep}
  \caption{Hit rate, cosine quality, and cost savings vs.\
  routing threshold $\theta$. Adaptive selection centers at
  $\theta = 0.772$ across five seeds.}
  \label{fig:router}
\end{figure}

\subsection{Cache Eviction Performance}
Despite the theoretical advantages of optimizing semantic coverage, the semantic eviction policy provided zero empirical benefit on the LMSYS workload.
...
\textbf{Runtime Overhead Breakdown:}
...
\subsection{Six-Policy, Three-Dataset Matrix}
We extend the eviction evaluation to all six policies on three workloads, each preprocessed into a 100{,}000-query temporal stream: LMSYS-Chat-1M, QQP \cite{iyer2017qqp} (Quora question pairs), chosen as a high-redundancy workload of near-duplicate questions, and MOSS \cite{sun2024moss}, an English synthetic-instruction corpus whose templated queries saturate any cache and serve as a control. Cache capacities are 10\%, 20\%, and 30\% of the stream, with three seeds (42, 123, 456) per cell. Figure~\ref{fig:heatmap} reports the full hit-rate matrix and Figure~\ref{fig:ablation} the capacity ablation.

LFU is never beaten: across all 9 dataset--capacity cells, no policy exceeds LFU's hit rate by more than 0.05\,pp. On the sparse workloads at 10\% capacity, LFU reaches 57.0\% (LMSYS) and 60.0\% (QQP). ARC ties LFU to within 0.01\,pp in every cell: its adaptation parameter only moves on semantic ghost hits, which are as rare as the redundancy signal itself, so ARC degenerates to its frequency list and inherits LFU's behavior. GDSF, as predicted by its reduction to LFU-with-aging, trails LFU by at most 0.48\,pp (QQP, 10\%) --- the aging clock's only effect --- and is otherwise indistinguishable.

SISO --- the one policy purpose-built for LLM caches --- is the \emph{worst} performer on both sparse datasets at every capacity, trailing LFU by 6.3, 4.2, and 1.4\,pp on LMSYS and by 8.5, 4.7, and 1.0\,pp on QQP at 10\%, 20\%, and 30\% capacity: its cluster-size signal suffers the same sparsity collapse as our semantic policy, while its maintenance rounds discard the frequency history that LFU exploits. Both deficits shrink monotonically with capacity, and the same convergence applies at the top of the ranking: LFU's lead over LRU falls from 1.5\,pp (LMSYS) and 2.5\,pp (QQP) at 10\% capacity to 0.2\,pp at 20\% and to below 0.01\,pp at 30\%. Capacity, not policy, dominates hit rate (Fig.~\ref{fig:ablation}); as the cache grows, every policy gap collapses toward zero.

MOSS behaves as intended for a control: all six policies land between 97.6\% (10\%) and 98.7\% (30\%), within 0.02\,pp of one another at each capacity --- a saturated cache is uninformative for policy ordering. Seed variance is negligible throughout: the maximum hit-rate standard deviation across all 54 policy cells is $2.2 \times 10^{-4}$ (0.02\,pp), so the orderings above are essentially deterministic. The hit-rate losses buy no speed either: at 10\% capacity, SISO's mean stream-processing time is $3.5\times$ LRU's on LMSYS and $3.1\times$ on QQP, with our semantic policy at $4.3\times$ and $3.8\times$ respectively; only on saturated MOSS, where evictions are rare, does the overhead collapse (SISO $1.3\times$ LRU).

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{phase6_policy_heatmap}
  \caption{Hit rate (\%) for all six policies across three
  ...
  0.05\,pp; SISO is worst in every LMSYS and QQP cell.}
  \label{fig:heatmap}
\end{figure}
...
$L_2^2$) is degenerate: on all three datasets, all six policies, and all three
seeds, the final hit rate is exactly $1.0$ with zero misses and zero
evictions. Because gte-base's random-pair similarity exceeds the threshold,
...
After per-encoder recalibration, the negative result replicates exactly. At
$10\%$ capacity the policy ordering on LMSYS and QQP is identical to MiniLM's:
$\text{ARC}=\text{LFU} > \text{GDSF} > \text{Semantic} > \text{LRU} >
\text{SISO}$. ARC and LFU are indistinguishable, matching within $0.01$pp in
all nine dataset--capacity cells. The semantic policy again never beats LFU.
SISO is again the only policy that separates, and again in the wrong
direction, trailing the best policy by $5.0$/$1.0$/$0.4$pp on LMSYS and
$8.3$/$4.8$/$1.2$pp on QQP at $10/20/30\%$ capacity, a deficit that narrows
as capacity grows. The LFU--LRU frequency advantage likewise shrinks with
...
served by another. Only the relative conclusion---frequency-based eviction
dominates, semantic and workload-aware alternatives add cost without
benefit---transfers.
...
\centering
\small
\input{paper/tables/phase8_judge_table}
\end{table*}
...
result: at the 0.90 hit threshold, most raw hits are not answers.
...
(27.0\%), because admitting-then-evicting quickly is no worse than
careful retention when retention preserves mostly non-equivalent hits.

Figure~\ref{fig:yesdist} exposes the mechanism: equivalence decays
with distance, but each dataset fails differently. MOSS falls from
74.5\% YES in its nearest distance decile to 2.4\% in its farthest.
QQP simply has no near-duplicate regime: its nearest decile already
sits at median $L_2^2 = 0.34$ and only 16.2\% YES. LMSYS is the
...
Near-zero embedding distance is not equivalence.
...
\section{Discussion: Analyzing the Negative Result}
The failure of the semantic eviction policy to outperform LFU is not an implementation flaw, but a structural property of the workload.
...
To locate where the redundancy signal dies, we profile density over the
\emph{cache contents} rather than the workload: at periodic snapshots of the
...
pairs, is the most redundant \emph{workload}---it shows the largest LFU--LRU
frequency payoff ($+2.49$pp, versus $+1.47$pp for LMSYS and $+0.00$pp for
saturated MOSS)---yet its cache \emph{contents} are the sparsest, $20\times$
below LMSYS and two orders of magnitude below MOSS.
...
inverse-utility ordering on \emph{every} dataset. The negative result is
...
\subsection{Workload Dependency and Applicability Boundaries}

The negative result on LMSYS-Chat-1M does not imply that semantic eviction is universally ineffective. Rather, it establishes a workload-dependency condition: semantic eviction provides benefit only when the inter-query redundancy graph is sufficiently dense. A practical deployment heuristic is to estimate redundancy statistics under the intended threshold before selecting a policy. If the measured mean redundancy is orders of magnitude smaller than $\mu$ (as in LMSYS, where mean $r(e)\approx2.5\times10^{-5}$ and $\mu=0.1$), the semantic term is effectively inactive and LFU should be preferred.

Clustered workloads where semantic eviction is likely to dominate include: repetitive corporate FAQ systems, automated homework or coding assessment platforms, and domain-specific RAG pipelines with constrained query vocabularies. General-purpose conversational AI deployments should use LFU.

\subsection{What Quality Adjustment Means for the Negative Result}
The judge results sharpen the negative result in two directions.
First, eviction policies are not merely tied on raw hit rate; they are
tied on the metric that matters, since no policy's judged-equivalence
...
return an answer to a different question. The operative lever in a
semantic cache is therefore not \emph{which entry to evict} but
\emph{which matches to accept} --- the hit threshold and the quality of
the match signal dominate any eviction decision made downstream of
them. This also closes the loop on the router's quality proxy: the
...
on pairs a judge deems non-equivalent, so similarity-as-quality is
itself an unreliable objective on boilerplate-heavy text.

\section{Limitations}

This study has five primary limitations. First, both embedding models are compact sentence encoders (\texttt{all-MiniLM-L6-v2}, 384 dimensions; \texttt{gte-base}, 768 dimensions). The cross-encoder replication shows the policy ordering is robust, but larger embedding models may produce different distance geometry, and our own results show that absolute hit rates do not transfer across encoders. Second, the three datasets, while spanning diverse and redundant workloads, are all English-language chatbot or question corpora; results may not generalize to specialized workloads such as code or domain-specific RAG. Third, latency benchmarks were conducted on CPU-only hardware; GPU-accelerated serving infrastructure may shift the relative overhead of semantic eviction. Fourth, multi-turn conversation context was excluded from the cache key, treating each turn as independent; context-aware caching may yield different hit-rate characteristics. Fifth, the judge itself is an 8B-parameter model and may misjudge subtle equivalences, although the near-identical verdicts from an independent hosted runtime ($\kappa = 0.826$) indicate the findings are not an artifact of one deployment. Query equivalence is likewise a proxy for the quantity we ultimately care about, response reusability: a non-equivalent query pair can occasionally share a serviceable answer. Finally, the judged sample covers the seed-42 MiniLM runs at 10\% cache capacity only, so the quality-adjusted rates are point estimates for one operating configuration rather than seed-averaged quantities.

\section{Conclusion}
We presented CLEVER, a framework for evaluating semantic caching architectures for Large Language Models. While HNSW indexing and adaptive cost-based routing provide significant, measurable benefits to latency and LLM serving costs, we demonstrate that semantic-aware cache eviction is highly dependent on workload distribution. On diverse, real-world conversational datasets like LMSYS-Chat-1M, the semantic redundancy graph is too sparse to provide actionable signal. This conclusion survives every escalation we applied: six policies, three workloads, three capacities, and two embedding encoders. LFU is never beaten; ARC converges to LFU's decisions; SISO, designed specifically for LLM serving, is the worst policy on both sparse workloads; and every policy gap shrinks monotonically as capacity grows. The LLM-judge audit goes further: most raw hits at the standard threshold are not semantically equivalent queries, so the hit rates the policies compete over are largely illusory, and quality adjustment erases the residual differences between them. The binding constraint in semantic caching is hit quality --- the threshold and the match signal --- not eviction. Practitioners should pair LFU with per-encoder threshold calibration and judged spot-checks of hit quality rather than invest in semantically informed eviction. Future work should investigate whether combinatorial or bandit-based semantic eviction \cite{liu2026semantic} can overcome sparsity limits through online adaptation, and whether response-grounded judging on workloads with stored responses corroborates the query-equivalence audit.
\section{Reproducibility and Artifact Availability}
All code, configuration files, and raw result artifacts used in this paper are included in the accompanying project repository. Core quantitative claims map directly to result files under \texttt{results/benchmarks/}, \texttt{results/routing/}, and \texttt{results/eviction/}. Runtime manifests record command lines, software versions, hardware metadata, and Git commit information to support replication.
...
This project was conducted as part of CSE 584 (Advanced Database Systems) at the University of Michigan. We thank the LMSYS dataset creators and the Great Lakes HPC infrastructure used for large-scale eviction experiments.
\vspace{-0.3cm}
\balance
