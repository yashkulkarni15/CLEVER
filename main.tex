...
\begin{enumerate}
\item \textbf{ANN Index Benchmark:} A systematic comparison of FAISS index structures (Flat, HNSW, IVF, LSH) under realistic semantic caching workloads on 499K vectors, establishing HNSW as the Pareto-optimal choice (Recall@1 $= 0.989$ at $P50 = 0.52$ ms, a $34\times$ speedup over exact search for a 1.1\,pp recall loss).
...
CLEVER is organized as a sequence of modular components: an index benchmark, a cost-based router, an eviction layer hosting six interchangeable policies, and an LLM-based audit of hit quality. This section describes each in turn.
...
All six policies (LRU, LFU, Semantic, ARC, GDSF, SISO) run under an identical harness: the policy only observes insert, access, and evict events and selects victims, while admission, lookup, the
...
Despite the theoretical advantages of optimizing semantic coverage, the semantic eviction policy provided zero empirical benefit on the LMSYS workload. Table~\ref{tab:hit-rate} reports hit rates for LRU, LFU, and the semantic policy on the full 579,753-query stream. At 10\% capacity, LFU leads the semantic policy by 0.27\,pp and LRU by 0.33\,pp; at 20\% and 30\% capacity, all three policies agree to four decimal places. Seed variance is below reporting precision, so these small differences are stable, but they carry no practical significance.
...
pairs, is the most redundant \emph{workload}, and it shows the largest
LFU--LRU frequency payoff ($+2.49$pp, versus $+1.47$pp for LMSYS and
$+0.00$pp for saturated MOSS). Yet its cache \emph{contents} are the
...
The judge results strengthen the negative result in two ways.
...
stand-in for answer quality on boilerplate-heavy text.
...
\section{Conclusion}
We presented CLEVER, a framework for evaluating semantic caching architectures for Large Language Models. While HNSW indexing and adaptive cost-based routing provide significant, measurable benefits to latency and LLM serving costs, we demonstrate that semantic-aware cache eviction is highly dependent on workload distribution. On diverse, real-world conversational datasets like LMSYS-Chat-1M, the semantic redundancy graph is too sparse to provide actionable signal. This conclusion held under every extension we tested: six policies, three workloads, three capacities, and two embedding encoders. No policy outperforms LFU; ARC converges to LFU's decisions; SISO, designed specifically for LLM serving, is the worst policy on both sparse workloads; and every policy gap shrinks monotonically as capacity grows. The LLM-judge audit adds a second, independent argument: most raw hits at the standard threshold are not semantically equivalent queries, so the hit rates the policies compete over are largely illusory, and quality adjustment erases the residual differences between them. What limits a semantic cache is the quality of its hits, set by the threshold and the match signal, rather than its eviction policy. Practitioners should pair LFU with per-encoder threshold calibration and judged spot-checks of hit quality rather than invest in semantically informed eviction. Future work should investigate whether combinatorial or bandit-based semantic eviction \cite{liu2026semantic} can overcome sparsity limits through online adaptation, and whether response-grounded judging on workloads with stored responses corroborates the query-equivalence audit.
