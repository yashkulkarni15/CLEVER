...
\title{CLEVER: Cache Self-Deduplication and Hit Quality Limit
Semantic Eviction in LLM Caches}
...
We report two positive systems results and two negative ones, and argue the negatives are the more consequential. On the positive side, HNSW is the Pareto-optimal index, and an adaptive cost-based router attains 60.33\% mean latency savings across five seeds at its cosine-similarity operating point. The negative findings concern eviction and hit quality. First, semantic-aware eviction does not help, and we identify the mechanism. Because cache hits are served without re-insertion, the cache self-deduplicates, so even highly redundant workloads yield a sparse redundancy graph with little signal for a semantic policy to exploit. Across six policies, three workloads, three capacities, and two encoders, no policy beats LFU by more than 0.05\,pp (a handful of queries out of ${\sim}70$K). ARC converges to LFU's decisions, GDSF reduces to LFU-with-aging, SISO (a policy designed specifically for LLM serving) is the worst policy on both sparse workloads, and every policy gap shrinks monotonically with capacity, effectively vanishing at a 30\% cache. The policy ordering replicates under a second encoder, although the hit thresholds themselves do not transfer and must be recalibrated per encoder. Second, an LLM judge, validated against an independently hosted judge, finds that most cache hits at the standard threshold are not semantically equivalent queries: quality-adjusted hit rates collapse from 51--60\% to 1--2\% on LMSYS and QQP and from 98\% to 25\% on MOSS, and the residual differences between policies fall within binomial noise. The semantic policy also adds 5.83--8.24$\times$ per-query overhead relative to LRU. We conclude that the binding constraints in semantic caching are cache self-deduplication and hit quality, not the eviction policy: LFU is the dominant, cost-effective choice, and effort is better spent on per-encoder threshold calibration and hit-quality validation than on semantically informed eviction.
\end{abstract}
\maketitle
...
\item \textbf{Six-Policy, Cross-Encoder Robustness Study:} An extension of the negative result to ARC, GDSF, and SISO across three workloads, three cache capacities, and two embedding models, showing that no policy beats LFU by more than 0.05\,pp, that all policy gaps vanish as capacity grows, and that semantic-cache hit thresholds do not transfer across encoders and must be recalibrated per encoder.
...
\textbf{Pipeline.} The core results are regenerated by \path{03_run_index_benchmark.py} (\path{configs/index_benchmark.yaml}); \path{06_run_routing_eval.py} \texttt{--config} \path{configs/routing.yaml} \texttt{--multi-seed} (five-seed adaptive router); \path{08_run_eviction.py} \texttt{--config} \path{configs/eviction.yaml} \texttt{--multi-seed} (three-policy full-LMSYS eviction); the six-policy matrix via \path{08_run_eviction.py} \texttt{--size 100k} \texttt{--multi-seed}, with \texttt{--dataset} set to each of \texttt{lmsys}, \texttt{qqp}, and \texttt{moss} and \texttt{--embedding-model} to each encoder, against \path{configs/eviction.yaml} (MiniLM) and \path{configs/eviction_gte.yaml} (recalibrated gte-base); and the judged-quality audit via \path{16_sample_hits_for_judge.py} $\rightarrow$ \path{17_run_llm_judge.py} $\rightarrow$ \path{18_visualize_judge.py}.
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.42\columnwidth}>{\raggedright\arraybackslash}p{0.50\columnwidth}@{}}
    ...
    Six-policy MiniLM matrix (Figs.~\ref{fig:heatmap},~\ref{fig:ablation}) & \path{results/eviction/phase6_matrix_*} \\
...
\textbf{Determinism and provenance.} Eviction cells run three seeds (42, 123, 456) and routing five (42, 123, 456, 789, 1024); the maximum hit-rate standard deviation across all policy cells is $2.2\times10^{-4}$, so orderings are effectively deterministic. The 30\% warmup prefix and per-encoder thresholds are fixed by config. Representative run commits are \texttt{39676dd} (routing), \texttt{e4f39ee} (three-policy eviction), and \texttt{d386ffa} (six-policy matrix); full per-run provenance, including working-tree state, is in each artifact's manifest. The index-benchmark artifact predates the manifest instrumentation and is regenerated deterministically from \texttt{03\_run\_index\_benchmark.py} and its config.
